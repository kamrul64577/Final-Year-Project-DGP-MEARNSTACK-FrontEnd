...
const AllOrders = () => {
    ...
    const handleDeleteOrder = id => {
        ...
        if (proceed) {
            const url = `https://desolate-beyond-74023.herokuapp.com/products/${id}`;
            fetch(url, {
                method: 'DELETE'
            })
                .then(res => res.json())
                        ...
                        allProducts.map(products =>
                            <tbody key={products._id}>
                                <tr>
                                    ...
                                    <td data-label="Action"><button onClick={() => handleDeleteOrder(products._id)} className="btn btn-danger">Delete</button><Link to={`/dashboard/updateProduct/${products._id}`}><Button className="text-light bg-dark">Update</Button></Link></td>
